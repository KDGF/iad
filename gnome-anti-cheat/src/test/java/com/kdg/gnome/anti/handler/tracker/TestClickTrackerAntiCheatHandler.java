 ...
 * 项目描述: 点击监测测试类
 *
 * 项目名称: TestClickTrackerAntiCheatHandler 
 * 创建日期: 2018年6月19日   
 * 修改历史：
 * 		1. [2018年6月19日]创建文件 by lh.qiu
 * java版本: JDK 1.8
 */
public class TestClickTrackerAntiCheatHandler extends AntiCheatHandlerBase {
	...
	public void testTrackerTimeout() {
		...
		// 判断监测是否过期
		AntiCheatStatus trackerCheat = ClickTrackerAntiCheatHandler.isTrackerCheat(trackerAntiCheatVO);
	...
	public void testTrackerRepeat() {
		...
		for (int times = 0; times < 2; times++) {
			AntiCheatStatus trackerCheat = ClickTrackerAntiCheatHandler.isTrackerCheat(trackerAntiCheatVO);
			// 输出监测状态
			System.out.println(String.format("点击监测次数：%s，监测状态：%s", times + 1, trackerCheat));
//			assertEquals("点击监测重复", AntiCheatStatus.NORMAL, trackerCheat);
		}
	}
	...
	public void testImp2ClickOverMin() throws InterruptedException {
		...
		// 调用点击监测
		AntiCheatStatus clickAntiStatus = ImpTrackerAntiCheatHandler.isTrackerCheat(trackerAntiCheatVO);
		...
		// 调用曝光监测的反作弊
		AntiCheatStatus impAntiStatus = ClickTrackerAntiCheatHandler.isTrackerCheat(trackerAntiCheatVO);
	...
	public void testTrackerIpFrequency() throws InterruptedException {
		...
		TrackerAntiCheatVO trackerAntiCheatVO = new TrackerAntiCheatVO();
		// 设置IP地址
		trackerAntiCheatVO.setIp("192.168.188.108");

		// 使IP被频次控制
		testTrackerIpFrequencyHidden(trackerAntiCheatVO);
		
		// 使线程休眠点击监测IP频次屏蔽过期秒数+1（使屏蔽过期）
		Thread.sleep(TimeUnit.SECONDS.toMillis(Constant.CLICK_TRACKER_IP_FREQUENCY_HIDEN_EXPIRE + 1));
		...
		AntiCheatStatus trackerCheat = ClickTrackerAntiCheatHandler.isTrackerCheat(trackerAntiCheatVO);
		// 输出监测状态
		System.out.println(String.format("点击频次过期后再次请求，监测状态：%s", trackerCheat));
		// 判断监测判断是否为normal
		assertEquals(AntiCheatStatus.CLICK_TRACKER_IP_FREQUENCY_HIDDEN, trackerCheat);
	}
	...
	private void testTrackerIpFrequencyHidden(TrackerAntiCheatVO trackerAntiCheatVO) {
		...
		// 点击监测IP频次最大数量+1（使请求超过限制）
		int maxTimes = Constant.CLICK_TRACKER_IP_FREQUENCY_MAX + 1;

		// 单IP循环请求点击监测P频次最大数量+1
